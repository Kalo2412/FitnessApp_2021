...
class AllTrainingsModel: ObservableObject {
    ...
    public func getAllTrainings() {
        ...
        if let currentUserUid = FirebaseManager.instance.auth.currentUser?.uid {
            FirebaseManager.instance.firestore.collection("users").document(currentUserUid).collection("trainings").getDocuments() { snapshot, error in
                if error != nil {
                    return
                }
